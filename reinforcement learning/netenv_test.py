...
class NetworkEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        # print(action, len(paths), blocking_action)

        # determine actions
        if action > blocking_action:   # error, block
            reward = blocking_reward
        elif action == blocking_action: # blocking
            # we need to block
            reward = blocking_reward
        else: # update edge stats       # select path
            ...
            data = {}
            # update based on holding time and measure util
            i = 0
            for e in self._graph.edges():
                count = 0
                for j in range(10):
